...
class TouchHandler {
    ...
    func touchMoved(_ touch: UITouch, key: KeyView, with event: UIEvent?) {
        ...
        case .nextKeyboard:
            guard let event = event, key.selectedAction == .nextKeyboard else { return }
            keyboardView?.delegate?.handleInputModeList(from: key, with: event)
        case .typing:
    ...
    func touchEnded(_ touch: UITouch, key: KeyView?, with event: UIEvent?) {
        ...
        case .cursorMoving:
            ...
            inputMode = .typing
        case .nextKeyboard:
            guard let key = key, let event = event else { return }
            keyboardView?.delegate?.handleInputModeList(from: key, with: event)
        case .typing:
            var inputKey = key
            ...
            case .keyboardType(.emojis), .character, .space, .newLine, .rime, .setChineseScript:
